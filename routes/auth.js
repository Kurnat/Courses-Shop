...
const sgMail = require('@sendgrid/mail'); // email sending
...
router.post('/register', registerValidators, async (req, res) => {
    try{
        ...
        if(!errors.isEmpty()){
            ...
            await user.save();
            res.redirect('/auth/login#login');
            //  send email to user about successful registration
            await sgMail.setApiKey(keys.SENDGRID_API_KEY);
...
router.post('/password', async (req, res)=>{
    try{
        ...
        if(user){
            ...
            req.flash('loginError', 'Password recovery time is out');
            res.redirect('/auth/login');
        }
    } catch(err){
        console.log(err);
...
router.post('/reset', (req, res) => {
    try{
        crypto.randomBytes(32, async (err, buffer) => {
            if(err) {
                req.flash('error', 'Someting went wrong, please try agein.'); 
